...
def main():
    ...
    print("\nCache Lab summary:")
    print(f"{'':22}{'Points':>8}{'Max pts':>10}{'Misses':>12}")
    print(f"{'Csim correctness':<22s}{csim_cscore[0]:8.1f}{maxscore['csim']:10d}")

    misses = str(miss32)
    if miss32 == 2**31-1 :
        misses = "invalid"
    print(f"{'Trans perf 32x32':22s}{trans32_score:8.1f}{maxscore['trans32']:10d}{misses:>12s}")

    misses = str(miss64)
    if miss64 == 2**31-1 :
        misses = "invalid"
    print(f"{'Trans perf 64x64':22s}{trans64_score:8.1f}{maxscore['trans64']:10d}{misses:>12s}")

    misses = str(miss61)
    if miss61 == 2**31-1 :
        misses = "invalid"
    print(f"{'Trans perf 61x67':22s}{trans61_score:8.1f}{maxscore['trans61']:10d}{misses:>12s}")

    total_max_score = maxscore['csim'] + maxscore['trans32'] + maxscore['trans64'] + maxscore['trans61']
    print(f"{'Total points':>22s}{total_score:8.1f}{total_max_score:10d}")
    
    # Emit autoresult string for Autolab if called with -A option
    if autograde:
        autoresult = f"{total_score:.1f}:{miss32:d}:{miss64:d}:{miss61:d}"
